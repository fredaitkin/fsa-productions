        ...
        <section class="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div class="space-y-6">
                ...
                <div class="flex flex-wrap gap-4">
                    <a href="/services" class="rounded-full bg-[#ffcc66] px-6 py-3 font-semibold text-[#08111f] transition hover:scale-[1.02] hover:bg-[#ffd98a]">Explore services</a>
                    <a href="/videos" class="rounded-full border border-white/15 bg-white/5 px-6 py-3 font-semibold text-white/90 transition hover:border-white/30 hover:bg-white/10">See video ideas</a>
                </div>
            </div>
            ...
            <div class="rounded-[2rem] border border-white/10 bg-white/8 p-6 shadow-2xl shadow-black/30 backdrop-blur-xl">
                ...
                <div class="mt-4 rounded-2xl border border-white/10 bg-gradient-to-br from-white/10 to-white/5 p-5">
                    ...
                    <ul class="mt-4 grid gap-3 text-sm text-white/80 sm:grid-cols-2">
                        <li v-for="video in videos" :key="video" class="rounded-full border border-white/10 bg-black/20 px-4 py-3">{{ video }}</li>
                    </ul>
                </div>
            </div>
        </section>
...
const videos = ['Brand films', 'Promo edits', 'Behind-the-scenes coverage', 'Social-first content'];
</script>